...
public class BuyingManager {
            ...
            MapManager    map)
    {
        ...
        for (Participant q : all) {
            if (q.core.getOwnedProperties()
                      .searchByLocation(p.position) != null)
            {
                owner = q;
                break;
            }
        ...
        if (owner == null) {
            System.out.print("Buy " + prop.getName() +
                             " for $" + prop.getPrice() + " (y/n): ");
        ...
        if (prop.getType().equals("property")) {
            if (ownsColourSet(p, prop) && !prop.isMortgaged()) {
                System.out.print("Build on " + prop.getName() +
                                 " for $" + prop.getHouseCost() + " (y/n): ");
                if (in.nextLine().equalsIgnoreCase("y")
                    && p.money >= prop.getHouseCost())
                {
                    if (prop.buildHouse()) {
                        p.money -= prop.getHouseCost();
                        System.out.println("Built on " + prop.getName() +
                                           ". Rent now $" + prop.getRent());
                    } else {
    ...
    /**
     * Very simple colour-set test: same first 3 letters, player owns ≥2.
     * Made public static so GameController can call it.
     */
