...
/**
 * A simple singly linked list to act as a deck of Cards.
 * Only a head pointer is used (no tail).
 */
class CardLinkedList {
    ...
    /**
     * Shuffle: copy to array, Fisher–Yates, then rebuild list.
     * Keeps only a head pointer (no tail).
     */
    public void shuffle() {
        /* 1) count nodes */
        int count = 0;
        ...
        if (count <= 1) return;  // nothing to shuffle

        /* 2) copy to array */
        Card[] arr = new Card[count];
        t = head;
        int idx = 0;
        while (t != null) { arr[idx] = t.card; t = t.next; idx++; }
